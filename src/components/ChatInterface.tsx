...
export const ChatInterface = ({
  ...
  // NEW: keep last submitted question so "Observations" shows the real question
  ...
  const generateRecommendations = (message: string) => {
    ...
    if (/math|equation|calculate|solve|integral|derivative/.test(lower)) {
      recs.push(
        "Try breaking the problem into smaller steps and check worked examples for similar problems."
      );
      recs.push(
        "Would you like a step-by-step walkthrough or a solved example?"
      );
    } else if (/science|physics|chemistry|biology/.test(lower)) {
      recs.push(
        "Consider drawing diagrams and identifying core principles involved."
      );
      recs.push(
        "I can provide experiment examples or key formulas if you want."
      );
    } else if (/history|war|century|period/.test(lower)) {
      recs.push(
        "Look for primary sources and timelines to understand context."
      );
      recs.push("I can summarize key events or provide recommended readings.");
    } else if (/grammar|write|essay|language/.test(lower)) {
      recs.push(
        "Try outlining your main points first and then expand each paragraph."
      );
      recs.push(
        "I can help edit or provide sample sentences if you share a draft."
      );
    } else {
      recs.push(
        "If you'd like, ask me for a summary, examples, or further practice problems."
      );
    }
    return recs;
  };
  ...
  const callOpenRouter = async (message: string, options?: CallOptions) => {
    ...
    if (options?.wiki) {
      messages.splice(1, 0, {
        role: "system",
        content: `Web search summary (Wikipedia): ${options.wiki}`,
      });
    }
    ...
    if (options?.thinkLonger) {
      messages.push({
        role: "system",
        content:
          "Please think longer about this question and provide detailed reasoning and additional examples.",
      });
      setThinkingNotes(
        "Thinking Longer is in development, please don't use this for now - Owner: Ram"
      );
    }
    ...
    try {
      ...
      const content: string =
        data?.choices?.[0]?.message?.content || "I couldn't find an answer.";
  ...
  const fetchWikipediaSummary = async (query: string) => {
    try {
      ...
      const opensearch = await fetch(
        `https://en.wikipedia.org/w/api.php?origin=*&action=opensearch&search=${search}&limit=1`
      );
  ...
  // Helper to get the most relevant "last user question" for the Observations block
  const getLastUserQuestion = () => {
    if (lastQuestion) return lastQuestion;
    // find last user message from chat history
    const lastUser = [...chat.messages]
      .reverse()
      .find((m) => m.role === "user");
    if (lastUser?.content) return lastUser.content;
    if (inputMessage && inputMessage.trim()) return inputMessage;
    return "...";
  };
  ...
  const sendMessage = async () => {
    ...
    // SAVE last question before clearing input so Observations displays it
  ...
  const sendThinkLonger = async () => {
    ...
    try {
      ...
      if ((err as any)?.name === "AbortError" || err.status === 0) {
        toast({
          title: "Thinking skipped",
          description: "Lala HAHAHAHHAHAHHAHA",
          variant: "default",
        });
      } else {
        toast({
  ...
  const useWebSearch = async () => {
    ...
    setIsLoading(true);
    onAddMessage(chat.id, {
      role: "user",
      content: `${query} (requested web search)`,
    });
    try {
      ...
      <ScrollArea ref={scrollAreaRef} className="flex-1 p-4 min-h-0">
        <div className="space-y-3 max-w-3xl mx-auto">
          {chat.messages.map((message) => (
            <div
              key={message.id}
              className={`group relative flex gap-2 animate-fade-in ${
            ...
            >
              ...
              >
                {message.role === "user" ? (
                  <div className="w-8 h-8 rounded-full flex items-center justify-center bg-chat-user text-chat-user-foreground font-semibold">
                    {(userName || "U").charAt(0).toUpperCase()}
                  </div>
                ) : (
                  <img
                    src="https://cdn.builder.io/api/v1/image/assets%2Ff7636dbc154444f9897eafaf4c70d8a5%2F72ff5047f88d49358f7660cd47a9a514?format=webp&width=800"
                    alt="AskEd"
                  ...
                  {message.role === "assistant" && (
                    <div className="ml-2 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-2">
                      <Button
                        ...
                        className="h-8 text-primary-foreground hover:bg-primary/10"
                      >
            ...
            <div className="flex gap-2 animate-fade-in items-start">
              ...
              <div className="max-w-[60%]">
                ...
                <div className="mt-2 flex items-center gap-2">
                  <button
                    onClick={() => setThinkingDetailsOpen((s) => !s)}
                    className="text-sm text-[hsl(210,90%,60%)] underline"
                  >
                    {thinkingDetailsOpen ? "Hide details" : "View thinking"}
                  </button>
                  <button
                ...
                {thinkingDetailsOpen && (
                  <div className="mt-2 p-2 rounded border text-sm bg-card/70">
                    <pre className="whitespace-pre-wrap">
                      {thinkingNotes ||
                        `Observations: The user asked: "${getLastUserQuestion()}"\nApproach: Identify key concepts, outline steps, provide examples, and summarize.`}
                    </pre>
                  </div>
                )}
              </div>
      ...
      <div className="border-t bg-card/50 backdrop-blur-sm p-4">
        <div className="max-w-4xl mx-auto">
          <p className="text-xs text-muted-foreground mb-2 text-center">
            AskEd can make errors, please double check the information.
          </p>
          {/* Toolbar: Think Longer / Web Search - centered and horizontal */}
          <div className="flex items-center justify-center gap-3 mb-3">
            <Button
              onClick={sendThinkLonger}
              disabled={!inputMessage.trim() || isLoading}
              variant="outline"
              size="sm"
            >
              Think Longer(Not Available)
            </Button>
            <Button
          ...
          <div className="flex gap-2 items-end">
            <div className="flex-1 relative">
              <Textarea
                ...
                placeholder={`Ask ${
                  userName ? `me anything, ${userName}` : "me anything"
                }...`}
            ...
            <Button
              onClick={sendMessage}
              disabled={!inputMessage.trim() || isLoading}
              className="bg-gradient-primary hover:shadow-glow transition-all duration-300 h-11 w-11"
            >
          ...
          <div className="text-xs text-muted-foreground mt-2 text-center">
            © {year} Quantara Corp. All rights reserved.
          </div>
        </div>
      </div>
    </div>
